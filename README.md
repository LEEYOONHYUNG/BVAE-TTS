...
## Pre-trained models  
We provide a pre-trained BVAE-TTS model, hich is a model that you would obtain with the current setting. (e.g. hyperparameters, dataset split) Also, we provide a pre-trained WaveGlow model that is used to obtain the audio samples. (We have been used an old version of WaveGlow, so we provide the WaveGlow source code and the pre-trained model together.)
